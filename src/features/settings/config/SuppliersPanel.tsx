        ...
        <div className="text-slate-200">
            ...
            {isAdding && (
                <div className="bg-slate-900 border border-slate-700 p-4 rounded-xl mb-6 animate-in slide-in-from-top-2">
                    <h4 className="font-bold mb-4 text-sm uppercase tracking-widest text-gold-500">New Supplier Details</h4>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div className="col-span-1">
                            <label className="text-xs text-slate-500 mb-1 block uppercase font-bold">Supplier Name</label>
                            <input
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(!suppliers || suppliers.length === 0) ? (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-800 rounded-2xl">
                        ...
                        <p className="text-slate-600 uppercase font-black tracking-[0.2em]">No Supplier Records Detected</p>
                    </div>
                ) : (
                    suppliers.map(supplier => (
                        <div key={supplier.id} className="bg-slate-900/50 border border-slate-800 p-4 rounded-xl flex items-start justify-between group hover:border-slate-600 transition-all">
                            {editingId === supplier.id ? (
                                ...
                                <>
                                    <div className="flex gap-4">
                                        ...
                                        <div>
                                            <div className="font-bold text-white group-hover:text-gold-500 transition-colors">{supplier.name}</div>
                                            <div className="text-xs text-slate-400 font-mono mt-0.5">{supplier.phone || 'No Phone'}</div>
                                    ...
                                    <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => startEdit(supplier)} className="p-2 hover:bg-slate-800 rounded text-slate-400 hover:text-white transition-all"><Edit2 size={14} /></button>
                                        <button onClick={() => deleteSupplier(supplier.id)} className="p-2 hover:bg-red-900/30 rounded text-red-500 transition-all"><Trash2 size={14} /></button>
                                    </div>
                                </>
                            )}
                        </div>
