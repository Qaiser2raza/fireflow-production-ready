...
import { X, Clock, Users, Receipt, Edit2, CheckCircle2, FileText, Printer } from 'lucide-react';
...
export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
    ...
    return (
            ...
            <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm">
                <div className="bg-[#0f172a] rounded-[32px] w-full max-w-3xl border border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-emerald-500 font-bold uppercase">
                                                        {order.payment_method === 'CREDIT'
                                                            ? `KHATA${order.customer_name ? ` — ${order.customer_name}` : ''}`
                    ...
                    <div className="bg-[#0a0e1a] p-6 border-t border-slate-800 space-y-3">
                        {/* Print row — always visible when order has items */}
                        {items.length > 0 && (
        ...
        `}</style>
            ...
            {/* Unified Receipt Modal — fetches fresh data from API using orderId */}
            {showReceiptPreview && (
                <ReceiptPreviewModal
                    ...
                    order={order}
                    orderId={order.id}
                    title={isPaid ? 'Reprint Receipt' : 'Print Bill'}
                />
            )}
        </>
    );
