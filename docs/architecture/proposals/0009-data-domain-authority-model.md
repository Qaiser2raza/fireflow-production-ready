...
source: Fable 5 C3 (authority model per data domain); ADR-001 refinement
sensitivity: internal
---

# ADR-0009 Draft — Authority Model per Data Domain

**Status:** DRAFT — for founder/CTO approval, then canonicalize into `docs/adr/`.
**Refines:** ADR-001 (hybrid architecture) without contradicting it.
**Motivation:** F-V15, G3, and TD-12 all trace to one ambiguity — which side of
the hybrid split is authoritative for each domain. This ADR names the rule once.

## Decision

Every data domain is assigned exactly one authority posture:
...
- **No client — anonymous or authenticated tenant — writes these tables,
  ever** (TD-12 lockdown enforces; RLS: reads scoped, writes service-role-only).
...
- Future domains (inventory, Cravex, AI memory) must declare their posture
