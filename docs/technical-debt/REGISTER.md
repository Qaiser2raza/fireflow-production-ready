...
| TD-12 | `restaurants_cloud` is anon-readable via the publishable client key: any browser visitor can enumerate tenant billing rows (`monthly_fee`, `currency`, `trial_ends_at`, plan, status). Harmless today (one demo row); becomes a tenant-billing exposure the moment real tenants sync on a hosted deployment. Fix: restrictive RLS (authenticated, owner-scoped reads); server-side sync is immune (service key bypasses RLS). Bundle with F-V3 Vault-licensing cloud-auth posture. | classified 2026-08-25 (F-V14 probe) | F-V14 / cloud transition | F-V3 decision |
