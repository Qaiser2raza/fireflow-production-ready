...
param (
    [SecureString]$DbPassword = (ConvertTo-SecureString "fireflow2026" -AsPlainText -Force),
    [string]$DbPort = "5432",
...
if (-Not (Test-Path $psql)) {
    ...
    Start-Process -FilePath $PgInstaller `
        -ArgumentList "--mode unattended --superpassword `"$PlainDbPassword`" --serverport $DbPort" `
        -Wait
    $env:Path += ";$pgBin"
...
Start-Process -FilePath $AppInstaller.FullName -Wait
...
DATABASE_URL="postgresql://postgres:$PlainDbPassword@localhost:$DbPort/$DbName?schema=public"
DATABASE_USER=postgres
DATABASE_PASSWORD=$PlainDbPassword
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host " DB Password : $PlainDbPassword"
